...
class BufferManager{
    ...
    Page insertIntoPool(string tableName, int pageIndex, bool LIFO);

    public:
    
    BufferManager();
    Page getPage(string tableName, int pageIndex);
    Page getPage(string tableName, int pageIndex, bool LIFO);
    void writePage(string pageName, vector<vector<int>> rows);
    void writePage(string tableName, int pageIndex, vector<int> row, int rowCount);
